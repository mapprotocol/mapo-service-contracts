module.exports = async (taskArgs, hre) => {
    ...
    await (
        await feeService.connect(deployer).setChainGasPrice(taskArgs.chainid, taskArgs.token, taskArgs.price)
    ).wait();

    console.log(
        `Fee service set the fee for ${taskArgs.chainid} ${taskArgs.token} to price is ${taskArgs.price} and  baselimit is ${taskArgs.base}`
    );
};
